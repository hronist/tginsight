...
export function SettingsMenu() {
  ...
  const isOpenRouter = settings.mode === "openrouter";
  const isCompatible = settings.mode === "compatible";
      ...
      >
        ...
        <span
          className={cn(
            ...
            keyConfigured || isCompatible ? "bg-primary" : "bg-amber-400",
          )}
        />
      </Button>
                ...
                <>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Label className="text-xs font-semibold">Ответы AI</Label>
                      <Badge
                        variant={
                          isOpenRouter
                            ? keyConfigured
                    ...
                    <div className="space-y-1.5">
                      ...
                      <ToggleGroup
                        value={[settings.mode === "proxy" ? "openrouter" : settings.mode]}
                        onValueChange={(v) => {
                      ...
                      <button
                        ...
                        disabled
                        title="Coming soon"
                        className="mt-1.5 flex h-8 w-full cursor-not-allowed items-center justify-center rounded-lg border border-input text-xs text-muted-foreground opacity-50"
                      >
                        Прокси · coming soon
                      </button>
                      <p className="text-[10px] leading-snug text-muted-foreground">
                        {isOpenRouter
                          ? "Ключ с openrouter.ai. Модель в формате openai/gpt-4o-mini."
                          : "OpenAI-совместимый endpoint: vLLM, Ollama, LM Studio, LiteLLM…"}
                      </p>
                    </div>

                    {isCompatible && (
                      <div className="space-y-1.5">
                        <Label htmlFor="settings-base-url" className="text-xs">
                          Base URL
                        </Label>
                        <Input
                        ...
                        />
                        <p className="text-[10px] leading-snug text-muted-foreground">
                          Корень API с /v1. Запрос уйдёт на …/chat/completions.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        placeholder={isOpenRouter ? "sk-or-…" : "опционально"}
                        autoComplete="off"
                      />
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-[10px] leading-snug text-muted-foreground">
                        WebGPU не всегда быстрее. Для Potion часто выгоднее WASM.
                      </p>
                    </div>
                  </div>
                  ...
                  <p className="rounded-lg border border-border bg-muted p-2 text-[10px] leading-relaxed text-muted-foreground">
                    ...
                    уходят только найденные фрагменты и ваш запрос. Свой API
                    (vLLM и т.п.) вызывается напрямую из браузера: нужен CORS на
                    сервере.
                  </p>
                </>
              )}
